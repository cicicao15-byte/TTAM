    ...
    >
      <div className="relative overflow-hidden rounded-lg border border-black/5" style={{ background: card.bg }}>
        <div className="aspect-[3/4] p-2">
          ...
          <div className="mb-1 text-[8px] font-semibold tracking-[0.02em]" style={{ color: card.accent }}>
            {card.title}
          </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-[14px] text-neutral-lowOnSurface">{open ? '^' : 'v'}</span>
      </button>
      {open && (
        <div
          className={[
  ...
  return (
    ...
    >
      ...
      >
        ...
        {children && <span className="text-[14px] text-neutral-lowOnSurface">{open ? '^' : 'v'}</span>}
      </button>
...
export default function CarouselTemplateEditor4() {
  const [drawerOpen, setDrawerOpen] = useState(true);
  const [activeTool, setActiveTool] = useState<ToolKey>('template');
  const [toolPanelVisible, setToolPanelVisible] = useState(true);
  ...
  const applySnapshot = (snapshot: EditorSnapshot, actionLabel: string) => {
    setActiveTool(snapshot.activeTool);
    setActiveLayerKey(snapshot.activeLayerKey);
    setSelectedTemplateId(snapshot.selectedTemplateId);
    setSelectedProductId(snapshot.selectedProductId);
    setBaseBackgroundId(snapshot.baseBackgroundId);
    setSelectedBackgroundId(snapshot.selectedBackgroundId);
    setSelectedTextPresetId(snapshot.selectedTextPresetId);
    setTextLayers(snapshot.textLayers.map((item) => ({ ...item })));
    setActiveTextLayerId(snapshot.activeTextLayerId);
    setBackgroundLayers(snapshot.backgroundLayers.map((item) => ({ ...item })));
    setActiveBackgroundLayerId(snapshot.activeBackgroundLayerId);
    setStickerLayers(snapshot.stickerLayers.map((item) => ({ ...item })));
    setActiveStickerLayerId(snapshot.activeStickerLayerId);
    setNextOverlayOrder(snapshot.nextOverlayOrder);
    setRemoveBackground(snapshot.removeBackground);
    setSafeZone(snapshot.safeZone);
    setTextPanelOpen({ ...snapshot.textPanelOpen });
    setExpandedLayers({ ...snapshot.expandedLayers });
    setStaticLayerOrders({ ...snapshot.staticLayerOrders });
      ...
      return (
        <>
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              <div className="text-[24px] font-semibold text-neutral-highOnSurface">Recommended templates</div>
          ...
          <>
            ...
            <div className="absolute inset-y-0 right-0 flex w-[1120px] max-w-full bg-neutral-surface shadow-[-12px_0_40px_rgba(0,0,0,0.14)]">
              <div className="flex w-[96px] shrink-0 flex-col border-r border-neutral-fillLow bg-neutral-surface px-3 py-4">
                ...
                <button
                  type="button"
                  aria-label={toolPanelVisible ? 'Collapse tool panel' : 'Expand tool panel'}
                  onClick={() => setToolPanelVisible((prev) => !prev)}
                  className="mb-[88px] mt-auto self-center rounded-[10px] border border-[#121415] bg-white p-0.5 text-[#121415] transition-colors hover:bg-neutral-surface1Hover"
                >
                  <span className="flex h-6 w-6 items-center justify-center rounded-[8px]">
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true">
                      {toolPanelVisible ? (
                        <>
                          <path d="M9.5 4.5L6 8L9.5 11.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                          <path d="M13 4.5L9.5 8L13 11.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                        </>
                      ) : (
                        <>
                          <path d="M6.5 4.5L10 8L6.5 11.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                          <path d="M3 4.5L6.5 8L3 11.5" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
                        </>
                      )}
                    </svg>
                  </span>
              ...
              <div className={`flex min-w-0 flex-1 pb-[76px] ${toolPanelVisible ? 'bg-[#ffffff]' : 'bg-[#f2f2f2]'}`}>
                {toolPanelVisible && (
                ...
                <div
                  className="flex w-[296px] flex-col bg-[#f2f2f2] py-5"
                  onClick={() => {
                    if (toolPanelVisible) {
                ...
                >
                  <div className="mx-3 flex min-h-0 flex-1 flex-col rounded-[16px] bg-[#ffffff] px-4 py-6 shadow-[0_8px_24px_rgba(15,23,42,0.06)]">
                    <div className="mb-5 flex items-center justify-between">
                      <div className="text-[18px] font-medium text-neutral-highOnSurface">layers and Settings</div>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          ...
                          onClick={() => {
                            ...
                            setRedoStack((prev) => [...prev.slice(-49), createSnapshot()]);
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            setUndoStack((prev) => [...prev.slice(-49), createSnapshot()]);
                            applySnapshot(nextSnapshot, 'Redo');
                          }}
                      ...
                      if (overlay.kind === 'text' && overlay.textLayer) {
                        ...
                        return (
                          <AccordionRow
                            ...
                            open={activeTextLayerId === textLayer.id}
                            ...
                            onClick={() => {
                              ...
                              setActiveTextLayerId((prev) => (prev === textLayer.id ? null : textLayer.id));
                            }}
                          >
                      ...
                      if (overlay.staticKey === 'decorationArrow') {
                        return (
                          <AccordionRow
                            key={overlay.id}
                            label="Decoration"
                            open={expandedLayers.decorationArrow}
                      ...
                      if (overlay.staticKey === 'decorationPattern') {
                        return (
                          <AccordionRow
                            key={overlay.id}
                            label="Decoration"
                            open={expandedLayers.decorationPattern}
                      ...
                      if (overlay.staticKey === 'decorationBand') {
                        return (
                          <AccordionRow
                            key={overlay.id}
                            label="Decoration"
                            open={expandedLayers.decorationBand}
                    ...
                    <BackgroundLayerRow
                      ...
                      open={activeLayerKey === 'backgroundPrimary' && activeBackgroundLayerId === null && expandedLayers.backgroundPrimary}
                      onClick={() => {
                        const isActiveBaseBackground = activeLayerKey === 'backgroundPrimary' && activeBackgroundLayerId === null;
                        ...
                        setExpandedLayers((prev) => ({
                          ...prev,
                          backgroundPrimary: isActiveBaseBackground ? !prev.backgroundPrimary : true,
                        }));
                      }}
                    >
                      <div className="grid grid-cols-5 gap-3">
                        ...
                        {BACKGROUND_ASSETS.map((presetAsset) => {
                          const presetActive = presetAsset.id === baseBackground.id;
                          ...
                          return (
                            <button
                              ...
                              onClick={(event) => {
                                ...
                                setActiveLayerKey('backgroundPrimary');
                                setActiveBackgroundLayerId(null);
                                setExpandedLayers((prev) => ({ ...prev, backgroundPrimary: true }));
                ...
                <div
                  className={[
                    'flex bg-[#f2f2f2] px-4 py-6',
                    toolPanelVisible ? 'w-[424px] flex-col items-center' : 'min-w-0 flex-1 justify-center',
                  ].join(' ')}
                >
